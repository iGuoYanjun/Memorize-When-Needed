...
def process_control_camera(control_camera_values):
    x = rearrange(control_camera_values, "b f c h w -> b c f h w")
    
    x = torch.concat(
        [
            torch.repeat_interleave(x[:, :, 0:1], repeats=4, dim=2),
            x[:, :, 1:]
        ],
        dim=2
    ).transpose(1, 2).contiguous()
    
    x = x.view(x.shape[0], x.shape[1] // 4, 4, x.shape[2], x.shape[3], x.shape[4])
    
    x = x.transpose(2, 3).contiguous()
    
    # 单行 view
    x = x.view(x.shape[0], x.shape[1], x.shape[2] * 4, x.shape[4], x.shape[5])
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(self, *args, **kwargs):
        raise NotImplementedError(
            "WanTransformer3DModel.forward is not used by infer_open_assets.py; "
            "use CustomWanTransformer3DModel.forward instead."
        )
    ...
    def from_pretrained(
        ...
        if os.path.exists(model_file):
            ...
            t1 = time.time()
            print(f">>> Loading {len(model_files_safetensors)} safetensors files...")
            for i, _model_file_safetensors in enumerate(model_files_safetensors):
                t_file = time.time()
                _state_dict = load_file(_model_file_safetensors)
                for key in _state_dict:
                    state_dict[key] = _state_dict[key]
                print(f">>>   [{i+1}/{len(model_files_safetensors)}] {os.path.basename(_model_file_safetensors)} loaded in {time.time() - t_file:.2f}s")
            print(f">>> All state dicts loaded in {time.time() - t1:.2f}s")
