...
def get_image_to_video_latent_fromPIL(validation_image_start, validation_image_end, video_length, sample_size):
    """
    支持:
      - validation_image_start: None 或 PIL.Image 或 [PIL.Image, ...]
      - validation_image_end:   None 或 PIL.Image 或 [PIL.Image, ...]
    返回:
      input_video:      [1, 3, T, H, W], 0~1
      input_video_mask: [1, 1, T, H, W], 0 或 255
      clip_image:       一张 PIL 图 (用于可视化)
    """
    # 工具函数: 把 PIL 图 resize 到 sample_size
    def _resize_img(img):
        return img.resize((sample_size[1], sample_size[0]))

    # 先处理 start 和 end, 统一成:
    #   start_list: None 或 list[PIL]
    #   end_list:   None 或 list[PIL]
    #   clip_image: 一张 PIL 或 None
    clip_image = None

    # ---------- 处理 start ----------
    if validation_image_start is None:
        start_list = None
    elif isinstance(validation_image_start, Image.Image):
        img = _resize_img(validation_image_start)
        start_list = [img]
        clip_image = img
    elif isinstance(validation_image_start, list):
        start_list = [_resize_img(im) for im in validation_image_start]
        clip_image = start_list[0] if len(start_list) > 0 else None
    else:
        raise TypeError(f"validation_image_start must be None or PIL.Image or list[PIL.Image], "
                        f"got {type(validation_image_start)}")

    # ---------- 处理 end ----------
    if validation_image_end is None:
        end_list = None
    elif isinstance(validation_image_end, Image.Image):
        img = _resize_img(validation_image_end)
        end_list = [img]
    elif isinstance(validation_image_end, list):
        end_list = [_resize_img(im) for im in validation_image_end]
    else:
        raise TypeError(f"validation_image_end must be None or PIL.Image or list[PIL.Image], "
                        f"got {type(validation_image_end)}")

    # ---------- 三种情况 ----------
    if start_list is not None and end_list is not None:
        # 有起始图和结束图
        # 1) 构造 start_video: [1, 3, T_start, H, W]
        start_video = torch.cat(
            [
                torch.from_numpy(np.array(im)).permute(2, 0, 1).unsqueeze(1).unsqueeze(0)
                for im in start_list
            ],
            dim=2,
        )
        # 2) 用第一帧扩展到 video_length
        input_video = torch.tile(start_video[:, :, :1], [1, 1, video_length, 1, 1])
        input_video[:, :, : len(start_list)] = start_video

        input_video_mask = torch.zeros_like(input_video[:, :1])
        input_video_mask[:, :, len(start_list):] = 255
        ...
        input_video[:, :, -end_video.shape[2]:] = end_video
        input_video_mask[:, :, -len(end_list):] = 0

        input_video = input_video / 255.0

    elif start_list is not None:
        # 只有起始图
        if len(start_list) > 1:
            # 多帧起始图
            start_video = torch.cat(
